...
def check(host):
    return subprocess.check_output(f'ping -c 1 {host}')
...
def trace_route(host):
    return subprocess.check_output(f'tracert -I -m 12 {host}')
    ...
    try:
        ...
        try:
            if not IPv4Address(message.text).is_global:
                ...
                messageID = await message.answer(f'Начинаю Трассеровку до {message.text}')
                await message.bot.send_chat_action(chat_id=message.chat.id, action=ChatAction.TYPING)
                await message.bot.edit_message_text(chat_id=messageID.chat.id,
                                                    text=trace_route(message.text).decode('IBM866'),
